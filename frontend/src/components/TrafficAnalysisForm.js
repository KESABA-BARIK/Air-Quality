    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!sourceCoords || !destinationCoords) {
                setResponseMessage("Could not find coordinates for source or destination.");
                return;
            }
            ...
            const aqiValues = aqiData.map(p => p.aqi).filter(aqi => typeof aqi === 'number' && aqi >= 0);
            const highAqiCount = aqiValues.filter(aqi => aqi >= 4).length;
    ...
    return (
        <div className="traffic-analysis">
            ...
            {routeCoords.length > 0 && (
                <>
                    <MapContainer center={routeCoords[0]} zoom={6} style={{ height: "400px", marginTop: "20px" }}>
                        
                        <TileLayer
                            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                            attribution="&copy; OpenStreetMap contributors"
                        />
                        <Polyline positions={routeCoords} color="blue" weight={5} opacity={0.7} dashArray="5, 10" />
...
const geocodeLocation = async (location) => {
    const url = `https://api.openrouteservice.org/geocode/search?api_key=${OPENROUTESERVICE_API_KEY}&text=${encodeURIComponent(location)}`;
    const res = await axios.get(url);
